...
dependencies point only backward, and every cross-reference is closed over the report and base
index.
...
`concept_assessment_links` by `(concept_node_id, role)`, `choice_diagnostics` by `choice_key`, and
`unresolved_issues` by `code`. Do not rely on generation order.

Before returning, inspect every ID array for duplicates and ordering, then validate the whole object
against the supplied JSON Schema and typed semantic constraints. `general_knowledge_used` is true
only when general model knowledge materially influenced a statement; record that influence only in
the fields allowed by the schema and never masquerade it as source evidence.
