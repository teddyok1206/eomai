...
if (kinds !== "paragraph,labeled_text,table,image,equation,paragraph,statement_set") {{
...
try {{
  orderedItemPreviewBlocks({{
    ...preview,
    blocks: [{{block_id: "block_future", type: "future"}}],
  }});
}}
...
def test_item_preview_dom_uses_safe_text_and_same_origin_media() -> None:
    ...
    assert "미리보기 준비 중" not in source
    assert "지원하지 않는 문항 형식" in source
    assert "block.label" in source
    assert ".innerHTML" not in source


def test_item_preview_clears_stale_hwpx_target_and_requires_capability_pair() -> None:
    source = (ROOT / "apps/web_gui/eom_web_gui/static/app.js").read_text(encoding="utf-8")
...
def test_item_preview_request_fails_closed_and_ignores_stale_responses() -> None:
    ...
    assert "itemPreviewRequestSequence: 0" in source
    assert "const requestSequence = ++state.itemPreviewRequestSequence;" in source
    assert source.count("requestSequence !== state.itemPreviewRequestSequence") == 2
    assert source.count("resetItemPreviewSelection(") == 3
