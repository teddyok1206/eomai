...
def _assert_v10_batch_executes_one_typed_identity_range_exactly_once(
    ...
    bootstrap_knowledge_analysis_control_plane(
        ...
        source_commit="a" * 40,
        actor_id="typed-identity-batch-integration",
        evaluation_cases_total=3,
        settings=orchestrator_settings,
    )
    document_revision_id = _document_source(
    ...
    with sessions() as session:
        ...
        assert request.source.page_image_count == 1
        assert request.worker_proposal_schema_ref.endswith("worker-proposal/5.0")

    _complete_proposal(
        integration_engine,
        catalog_settings,
        run_id=run_id,
        staging_root=tmp_path / "v10-batch-proposal",
    )
    _make_submitted_range_due(integration_engine, range_id=range_id)
    assert service.advance_once(runner_id="v10-batch-runner")
