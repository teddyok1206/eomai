...
def _receipts_v3() -> tuple[ArtifactPointer, ArtifactPointer, EvidenceUsageReceiptPair]:
    authoring_pointer = AUTHORING.model_copy(update={"result_schema": "authoring-result@12.0"})
    review_pointer = REVIEW.model_copy(update={"attempt": 2, "result_schema": "review-result@12.0"})
    manifest_pointer = _material_pointer(manifest=True)
    manifest_pointer["schema_ref"] = "eom://schemas/knowledge/evidence-bundle-manifest/5.0"
    shared: dict[str, object] = {
    ...
    authoring = AuthoringEvidenceUsageValidationReceiptV3.model_validate(
        {**authoring_value, "receipt_sha256": content_sha256(authoring_value)}
    )
    review_value = {
        **shared,
    ...
    review = ReviewEvidenceUsageValidationReceiptV3.model_validate(
        {**review_value, "receipt_sha256": content_sha256(review_value)}
    )
    return (
        authoring_pointer,
...
def test_v3_receipt_record_binds_verification_planned_escalated_review() -> None:
    authoring_pointer, review_pointer, pair = _receipts_v3()
    authoring_records = _record_set(authoring_pointer, pair.authoring)
    review_records = _record_set(review_pointer, pair.review)
    ...
    resolved = EvidenceUsageReceiptPair(
        authoring=cast(AuthoringEvidenceUsageValidationReceiptV3, authoring),
        review=cast(ReviewEvidenceUsageValidationReceiptV3, review),
    )
    OrchestratorEvidenceUsageReceiptResolver._validate_pair(resolved)

    assert review_pointer.attempt == 2
    assert resolved.review.result_artifact.result_schema == "review-result@12.0"
    assert resolved.review.authoring_artifact == resolved.authoring.result_artifact
    assert resolved.review.independent_review_report_sha256 == "sha256:" + "9" * 64
