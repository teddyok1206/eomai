...
def test_systemd_authorization_verifier_is_nonprivileged_and_negative_by_default() -> None:
    ...
    assert "eom-worker-document-review-06@.service" in source
...
def test_worker_runtime_deployer_installs_exact_pdf_document_review_template() -> None:
    source = (ROOT / "scripts/workflow/deploy_worker_runtime.sh").read_text(encoding="utf-8")

    assert '"${REPOSITORY}/infra/systemd/eom-worker-document-review-06@.service"' in source
    assert '"${UNIT_ROOT}/eom-worker-document-review-06@.service"' in source
    assert "PDF document-review worker unit source drift" in source
...
def test_runner_configuration_installs_root_owned_capability_policy() -> None:
    ...
    assert "config/workflows/knowledge-analysis.v10.yaml" in source
    ...
    assert "config/workflows/customer-support.v1.yaml" in source
    assert "/customer-support.yaml" in source
    ...
    assert "content/prompt-templates/placeholders/customer-support.txt" in source
    assert "/customer-support.txt" in source
