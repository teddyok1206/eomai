...
def test_mvp_control_plane_migration_is_additive_and_fail_closed() -> None:
    ...
    assert CURRENT_MIGRATION_REVISION == "20260909_0033"
...
def test_codex_usage_observation_migration_is_index_only_and_reversible() -> None:
    source = Path("migrations/versions/20260905_0029_codex_usage_observation.py").read_text(
        encoding="utf-8"
    )
    assert 'down_revision: str | None = "20260904_0028"' in source
    assert "ix_codex_control_command_latest_usage" in source
    assert "result_document ->> 'schema_version'" in source
...
def test_assessment_hwpx_build_migration_is_pointer_only_and_fifo_indexed() -> None:
    source = Path("migrations/versions/20260907_0031_hwpx_assessment_assembly_builds.py").read_text(
        encoding="utf-8"
    )
    assert 'down_revision: str | None = "20260906_0030"' in source
    assert "UPDATE assessment_" not in source
    assert "DELETE FROM" not in source
    table = Base.metadata.tables["hwpx_assessment_assembly_builds"]
    assert all(not isinstance(column.type, LargeBinary) for column in table.columns)
    assert not {"payload", "content", "bytes", "nas_path", "path"}.intersection(
        table.columns.keys()
    )
    indexes = {str(index.name): index for index in table.indexes if index.name is not None}
