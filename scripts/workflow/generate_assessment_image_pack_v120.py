...
LEGACY_SUBJECT_POLICY = """로컬 GPU 모델에는 위 원문 전체를 잘라서 보내지 않는다. Catalog가 원문과 drawing 전체를 immutable
...
SUCCESSOR_SUBJECT_POLICY = """로컬 GPU 모델에는 위 원문 전체를 잘라서 보내지 않는다. Catalog가 원문과 drawing 전체를 immutable
hash로 보존한 상태에서, image worker가 작성한 `drawing.alt_text`를 GPU의 짧은 의미 주제로 사용한다.
...
def _generate(target: Path) -> None:
    ...
    profile["profile"]["version"] = "12.2.0"
    profile_path.write_text(
        yaml.safe_dump(profile, allow_unicode=True, sort_keys=False), encoding="utf-8"
    )

    image_path = target / "prompt-templates/image.md"
    image_prompt = image_path.read_text(encoding="utf-8")
    if image_prompt.count(LEGACY_SUBJECT_POLICY) != 1:
        raise ValueError("legacy local image subject policy is not exact")
    image_path.write_text(
        image_prompt.replace(LEGACY_SUBJECT_POLICY, SUCCESSOR_SUBJECT_POLICY).rstrip()
        + "\n\n"
        + IMAGE_POLICY.strip()
        + "\n",
        encoding="utf-8",
