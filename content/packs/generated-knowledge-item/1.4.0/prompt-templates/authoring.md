...
scientific_constraints와 required_labels를 빠짐없이 기록하라. 정확한 수치·축·도형·회로·라벨은
항상 DETERMINISTIC_SVG를 사용한다. 사람·생물·자연 장면처럼 자연스러운 비권위적 배경이 실제로
도움이 되는 vector scene만 LOCAL_GENERATIVE_BACKGROUND를 선택할 수 있다. 이때에도 정답 판단에
필요한 모든 라벨·기호·수치·경계·과학 구조는 SVG overlay에 직접 정의하고, 생성 배경에는 어떤
문자·숫자·기호·측정 눈금도 맡기지 마라. HUMAN_REVIEWED_BACKGROUND는 아직 실행할 수 없다.
