...
ID는 정확히 일치시킨다. single_choice 문항이므로 correct_choice_ids에는 실제 5개 선택지 중 정확히
하나의 ID만 넣고 accepted_answers는 반드시 빈 배열로 둔다. statement_explanations는 ㄱ/ㄴ/ㄷ의
statement_id를 각각 정확히 한 번 포함한다. 결과 envelope의 고정 ID를 그대로 사용하고 completed_at은
현재 UTC RFC3339 시각으로 기록하라.
