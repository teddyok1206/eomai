...
실행 workspace의 다음 세 파일을 순서대로 처음부터 끝까지 읽어라.
...
앞의 두 파일은 서로 분리할 수 없는 콘텐츠팀장 문항·HwpQuestionEditor 계약이고, 셋째 파일은 검토된
삽화 실행 가이드다. 하나라도 없거나 읽을 수 없으면 결과를 만들지 말고 역할 실행을 명시적으로
실패시켜라. 요약본이나 기억으로 대체하거나 별도의 내용·양식 규칙을 추가하지 마라.

BEGIN_PINNED_LOCAL_IMAGE_PROVIDER_JSON
{{ local_image_provider.reviewed_binding_json }}
END_PINNED_LOCAL_IMAGE_PROVIDER_JSON

아래 authoring-result@12.0의 ordered visuals에서 IMAGE 슬롯만 원래 ordinal과 label 순서대로 찾는다.
각 IMAGE 슬롯마다 정확히 하나의 drawing을 반환하고 TABLE 슬롯은 그리지 마라. illustration_prompt는
팀장 원문이 요구한 다음 문장으로 반드시 시작한다.

`drawing.visual_ordinal`은 사람이 읽는 1번·2번 번호가 아니라 authoring draft의 `visuals` 배열에서
해당 IMAGE가 차지하는 **0부터 시작하는 배열 인덱스**다. 첫 `visuals` 원소의 값은 반드시 `0`, 둘째
원소의 값은 `1`이다. IMAGE 앞에 TABLE이 있으면 그 TABLE도 배열 위치에는 포함하므로 뒤 IMAGE의
인덱스가 증가한다. 배열을 처음부터 순회해 실제 인덱스와 label을 그대로 복사하고, IMAGE 개수나
표시 순서를 1부터 세어 새 값을 만들지 마라.

두 팀장 원문의 HwpQuestionEditor 책임 분리를 다음처럼 정확히 보존한다.

- IMAGE가 하나이면 하나의 PNG만 만들고 drawing label은 빈 문자열이다. `(가)` 또는 `(나)`를 PNG에
  그리거나 HWPX용 패널 label로 새로 만들지 마라.
- IMAGE가 둘이면 visual ordinal 0과 1에 서로 다른 PNG를 하나씩 만든다. drawing의 label은 각각
  `(가)`, `(나)`이지만 이 패널 label은 HWPX가 두 칸의 별도 편집 가능한 텍스트 행에 기록한다. 따라서
  `(가)`, `(나)`를 어느 PNG의 픽셀, SVG overlay 또는 GPU 배경에도 넣지 마라.
- IMAGE와 TABLE이 섞이면 실제 배열 ordinal은 그대로 보존하고 두 항목 모두 패널 label 없이 둔다.
- `A`, `B`, `P`, `Q`, 축 이름, 값처럼 과학 의미를 가진 required label은 패널 label과 다르다. 이들은
  GPU가 아니라 검증되는 deterministic SVG overlay에만 정확히 기록한다.
