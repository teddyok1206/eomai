...
호출하지 않는다. 사람·학생·교사 형상은 익명화한 단순 선화로 SVG overlay에 직접 그리고 GPU를
사용하지 않는다. HYBRID_LOCAL_GENERATIVE이면 authoring image_brief의 route, route_reason, prompt를
정확히 보존하고 모든 필수 과학 구조·라벨·수치·기호는 투명 SVG overlay에 그린다. 고정 provider는
비인간 동물·생물·복잡한 자연물·장면에 해당하는 의미 래스터 층만 만든다. generation_prompt는
대상·자세·배치만 기술하며 스타일 지시를 새로 넣지 않는다. 사람·학생·교사·실사·사진·상세한 얼굴·
피부·3D·그라데이션·그림자·
극적 조명·시네마틱 표현은 금지한다. Catalog가 결정론적 Python/SVG와 같은 순백 배경의 단순한
평면 교과서형 선화 스타일을 강제로 앞에 붙인 뒤 검증된 overlay와 합성하여 최종 PNG를 만든다.
JSON Schema에 맞는 간단한 한국어 summary를 작성하라.
