"""Idempotent control-plane bootstrap for isolated editorial compatibility."""
...
class LegacyItemEditorialCompatibilityBootstrapManifest(BaseModel):
    ...
    compatible_workflow_protocols: tuple[Literal["workflow-role/1.16.0"], ...] = Field(
...
class LegacyItemEditorialCompatibilityBootstrapResult(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
def load_legacy_item_editorial_compatibility_bootstrap_manifest(
    config_directory: Path,
) -> LegacyItemEditorialCompatibilityBootstrapManifest:
    ...
    try:
        ...
        raise ControlPlaneError(
            "CONTROL_BOOTSTRAP_INVALID", "editorial compatibility bootstrap manifest is invalid"
        ) from exc


def bootstrap_legacy_item_editorial_compatibility_control_plane(
    engine: Engine,
    *,
    config_directory: Path,
    source_commit: str,
...
) -> LegacyItemEditorialCompatibilityBootstrapResult:
    """Publish an evaluated source-only preset pinned to slot05 without executing Codex."""
    ...
    manifest = load_legacy_item_editorial_compatibility_bootstrap_manifest(config_directory)
    ...
    with transaction(sessions) as session:
        ...
        ensure_protocol_version(
            ...
            role_schema_bundle_hash("workflow-role/1.16.0"),
        )
    capacity_revision_id = _publish_editorial_compatibility_capacity_policy(
    ...
    instruction = _publish_instruction_bundle(
        ...
        identity_key="legacy-item-editorial-compatibility:support",
        bundle_key="legacy-item-editorial-compatibility-support",
    ...
    if draft.state == "RELEASED":
        with sessions() as session:
            ...
            if evaluation is None:
                raise ControlPlaneError(
                    ...
                    "released editorial compatibility preset lacks evaluation evidence",
                )
        released = draft
    else:
        ...
        report_artifact = publisher.publish_bytes(
            payload=payload,
            logical_name="legacy-item-editorial-compatibility-preset-evaluation.json",
    ...
    return LegacyItemEditorialCompatibilityBootstrapResult(
...
def _publish_editorial_compatibility_capacity_policy(
    ...
    document: dict[str, object] = {
        ...
        "created_at": EDITORIAL_COMPATIBILITY_CAPACITY_CREATED_AT.isoformat().replace(
            "+00:00", "Z"
        ),
    }
...
def _find_or_create_draft(
    ...
    *,
    manifest: LegacyItemEditorialCompatibilityBootstrapManifest,
    role_policies: list[dict[str, object]],
    capacity_policy_revision_id: str,
    actor_id: str,
) -> ExecutionPresetRevisionRecord:
    ...
    with transaction(sessions) as session:
        ...
        if logical is not None and logical.state != "ACTIVE":
            raise ControlPlaneError(
                "CONTROL_PRESET_RETIRED", "editorial compatibility preset is retired"
            )
        if logical is not None and logical.current_revision_id is not None:
            ...
            if (
                ...
                raise ControlPlaneError(
                    ...
                    "released editorial compatibility preset pointer differs",
                )
...
def _require_exact_six_slot_registry(slots: tuple[WorkerSlot, ...]) -> None:
    ...
    if actual != expected:
        raise ControlPlaneError(
            ...
            "editorial compatibility requires the exact six-slot inventory",
        )
