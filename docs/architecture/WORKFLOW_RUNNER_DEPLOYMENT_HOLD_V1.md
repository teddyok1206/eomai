...
unit as a reboot fence, loads the exact hold, then re-enables the unit only under that proven hold.
...
content hash, `RefuseManualStart`, `NeedDaemonReload=no`, and quiescent process state. The installer
also pins `InvocationID` and `ActiveEnterTimestampMonotonic` before and after hold activation so a
brief intervening execution fails closed. An unrelated or tampered path is never overwritten or
removed. The ineffective legacy runtime-mask symlink may be removed only after it is proven to be
...
8. **Transaction and concurrency.** No wheel, migration, DB, worker, or model action begins until
   ...
   revision, request, plan, operator, and checkpoint pointers. It admits only an ordered unique
...
10. **Failure, retry, and idempotency.** Exact existing hold bytes are accepted on retry. Partial,
    ...
    release never starts the runner. The reviewed backup supports retry before or after reload, and
    a retry after completed backup removal is an exact disabled-state no-op.
...
1. Prove the runner is `loaded/inactive/dead/MainPID=0`.
2. Activate and verify the persistent hold.  Only then remove a proven ineffective runtime-mask
...
5. As `eom-api`, publish the successful `retire-items` JSON envelope atomically to the fixed
   ...
   removes only the exact hold and leaves the runner inactive and disabled without changing its
   invocation identity.
